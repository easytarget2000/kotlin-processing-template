...
class JuliaSetRealm(maxColorValue: Float = 1f, random: Random = Random.Default): Realm(random) {
    ...
    override fun setup(pApplet: PApplet, pGraphics: PGraphics) {
        ...
        val scaleWidth = random.nextFloatInRange(from = 2.5f, until = 7f) // 5.5, 5.13, 2.23
        val scaleHeight = (scaleWidth * height.toFloat()) / width.toFloat() // 3.4, 3.21, 1.39
        val angle = random.nextFloatInRange(from = 3f, until = PConstants.TWO_PI) // 0.5, 6.2, 4.19
        val angleVelocity = random.nextFloatInRange(from = -0.05f, until = 0.05f) // 0.09, -0.29, -0.28
        val aAngleFactor = random.nextFloatInRange(from = 1f, until = 2f) // 1.69, 0.8, -1.35
